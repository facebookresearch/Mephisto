...
from mephisto.abstractions.providers.mturk.mturk_utils import get_assignments_for_hit, get_outstanding_hits
...
def main():
    ...
    completed_agentless_units = [u for u in units if u.get_status() in ['completed', 'accepted', 'soft_rejected'] and u.get_assigned_agent() is None]
    completed_agented_units = [u for u in units if u.get_status() in ['completed', 'accepted', 'soft_rejected'] and u.get_assigned_agent() is not None]
    completed_timeout_units = [u for u in completed_agented_units if u.get_assigned_agent().get_status() == 'timeout']
    ...
    print(
        f"Querying assignments for the {len(hits)} tasks."
    )
    ...
    do_cleanup = input(f"If all are reconciled, would you like to clean up remaining timeouts? y/(n)")
