...
class ChatApp extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      agent_status: props.agent_id.startsWith("onboarding")
        ? STATUS_ONBOARDING
        : STATUS_WAITING,
      done_text: null,
  ...
  componentDidUpdate(prev_props, prev_state) {
    if (
      prev_props.agent_id.startsWith("onboarding") &&
      !this.props.agent_id.startsWith("onboarding")
    ) {
      getInitTaskData(
    ...
    if (
      this.state.agent_status == STATUS_IN_TASK &&
      prev_state.agent_status == STATUS_WAITING
    ) {
  ...
  handleAgentStatusChange(agent_status, display_name, done_text) {
    ...
    if (agent_status != this.state.agent_status) {
      ...
      if (
        this.state.agent_status == STATUS_ONBOARDING &&
        agent_status == STATUS_WAITING
      ) {
        this.props.transferFromOnboarding();
      } else if (
        [STATUS_COMPLETED, STATUS_PARTNER_DISCONNECT].includes(agent_status)
      ) {
  ...
  render() {
    ...
    socket_handler = (
      <SocketHandler
        ...
        onForceDone={() =>
          postCompleteTask(this.props.agent_id, this.state.messages).then(() =>
      ...
      <div style={preview_style}>
        <div
          dangerouslySetInnerHTML={{
            __html: this.props.task_config.task_description,
          }}
        />
        ;
      </div>
...
class MainApp extends React.Component {
  ...
  afterAgentOnboarding() {
    completeOnboarding(this.state.agent_id).then((data) =>
  ...
  handleIncomingTaskConfig(task_config) {
    ...
    if (task_config.block_mobile && isMobile()) {
      ...
      requestTaskHMTL("preview.html", (data) =>
        this.handleIncomingPreviewHTML(data)
      );
    }
