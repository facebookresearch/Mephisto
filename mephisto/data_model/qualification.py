...
def is_valid_qualification_dict(qual_dict: Dict[str, Any]) -> bool:
    ...
    """
    required_keys = ['qualification_name', 'comparator', 'value', 'applicable_providers']
    ...
    if comparator not in SUPPORTED_COMPARATORS:
        raise AssertionError(f"Qualification comparator '{comparator}' not in supported list: {SUPPORTED_COMPARATORS}'")

    value = qual_dict['value']
    
    if comparator in [
        QUAL_GREATER, QUAL_LESS, QUAL_GREATER_EQUAL, QUAL_LESS_EQUAL
    ] and type(value) != int:
        raise AssertionError(f"Value {value} is not valid for comparator {comparator}, must be an int")

    if comparator in [QUAL_EXISTS, QUAL_NOT_EXIST] and value is not None:
        raise AssertionError(f"Value {value} is not valid for comparator {comparator}, must be None")

    if comparator in [QUAL_IN_LIST, QUAL_NOT_IN_LIST] and type(value) != list:
        raise AssertionError(f"Value {value} is not valid for comparator {comparator}, must be a list")

    if qual_dict['applicable_providers'] is not None:
        assert type(qual_dict['applicable_providers']) == list, "Applicable providers must be a string list of providers or none."
        valid_providers = get_valid_provider_types
        for provider_name in qual_dict['applicable_providers']:
            assert provider_name in valid_providers, (
