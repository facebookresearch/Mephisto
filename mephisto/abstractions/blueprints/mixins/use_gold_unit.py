...
class UseGoldUnitArgs:
    ...
    max_gold_units: int = field(
        ...
        metadata={
            "help": (
                "The maximum number of gold units that can be launched with this batch, "
                "specified to limit the number of golds you may need to pay out for."
            )
        },
    )
...
class UseGoldUnit(BlueprintMixin):
    ...
    def assert_mixin_args(cls, args: "DictConfig", shared_state: "SharedTaskState"):
        ...
        assert args.task.allowed_concurrent == 1, (
            "Can only run this task type with one allowed concurrent unit at a time per worker, "
            "to ensure golds are completed in order."
        )
        assert (
        ...
        # TODO (#97): it would be nice to test that `get_gold_for_worker`
        #  actually returns a task when given a worker
    ...
    def get_completion_stats_for_worker(self, worker: "Worker") -> Tuple[int, int, int]:
        ...
        correct_golds = UseGoldUnit.get_current_qual_or_default(
            worker,
            self.golds_correct_qual_name,
        )
        incorrect_golds = UseGoldUnit.get_current_qual_or_default(
            worker,
            self.golds_failed_qual_name,
        )
