 ...
 * @param {HTMLElement[]} formFieldElements HTML elements
 * @param {object} fields FormComposer fields
 * @return {object} invalid FormComposer fields
 */
export function validateFormFields(formFieldElements, fields) {
  const invalidFormFields = {};

  formFieldElements.forEach((formFieldElement) => {
    const field = fields[formFieldElement.name];
    ...
    Object.entries(fieldValidators).forEach((validator) => {
      ...
      const validationResult = validatorFunction(field, formFieldElement, ..._validatorArguments);
