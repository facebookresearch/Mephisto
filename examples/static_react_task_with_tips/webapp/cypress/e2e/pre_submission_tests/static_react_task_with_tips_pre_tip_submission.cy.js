...
const classNamePrefix = describe("Loads static_react_task_with_tips", () => {
  ...
  it("Loads correct tip react elements", () => {
    cy.visit("/");
    cy.get(".mephisto-worker-addons-tips__button");
  });
});
...
describe("Tips Popup", () => {
  it("Opening/Closing tips popup", () => {
    cy.visit("/");
    cy.get(".mephisto-worker-addons-tips__button").as("tipsButton");
    cy.get("@tipsButton").click();

    cy.get(".mephisto-worker-addons-tips__container").as("tipsContainer");
    ...
    cy.get(".mephisto-worker-addons-tips__button").should("be.disabled");
  ...
  it("Checking if tips header is too long", () => {
    ...
    cy.get("#mephisto-worker-addons-tips__header-input").as("tipsHeaderInput");
    cy.get("@tipsHeaderInput").type("This header is tooooooooooooooooo long");
    cy.get(".mephisto-worker-addons-tips__red-box").should(
      "have.text",
      "📝 Your tip header is too long"
    );
    cy.get(".mephisto-worker-addons-tips__button").should("be.disabled");
    cy.get("@tipsHeaderInput").clear();
    cy.get(".mephisto-worker-addons-tips__red-box").should("not.exist");
  });
  ...
  it("Checking if tips body is too long", () => {
    ...
    cy.get("#mephisto-worker-addons-tips__text-input").as("tipsBodyInput");
  ...
  it("Submitting a tip", () => {
    ...
    cy.get("@submitButton").should("be.disabled");
    cy.get(".mephisto-worker-addons-tips__green-box").should(
      "have.text",
